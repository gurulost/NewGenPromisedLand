...
Live production multiplayer gates run against `https://covenantlegends.com` by default and create a temporary public-authoritative lobby. The standard smoke covers three browser agents for four rounds. The soak gate defaults to four browser agents for ten rounds, supports up to eight players, writes artifacts under `output/live-multiplayer-soak/`, and injects browser reload/rejoin, active-player disconnect/reconnect, original-host browser leave plus host-lease claim, and an eliminated-player turn-handoff canary. These commands require the deployed bundle preflight to pass before any live users or lobbies are created.

The live smoke should use a bounded test-user pool in production verification when possible. Set `COVENANT_SMOKE_USER_PASSWORD` to create/reuse deterministic accounts such as `cltsmkp1` through `cltsmkp8` for smoke and `cltsoakp1` through `cltsoakp8` for soak, or set `COVENANT_SMOKE_USERS` to a JSON array of `{ "username": "...", "password": "..." }` entries for fully managed accounts. Without either setting, the script falls back to tagged run-scoped accounts with the `clt` prefix and records the cleanup note in `report.json`.
